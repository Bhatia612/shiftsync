...
const requireTeamMember = async (req, res, next) => {
    const { teamId } = requireTeamMember.params;

    const membership = await prisma.membership.findUnique({
        where: { userId_teamId: { userId: requireTeamMember.user.id, teamId } }
    })
...
const requireTeamManager = async (req, res, next) => {
    ...
    const membership = awaitprisma.membership.findunique({
        where: { userId_teamId: { userId: req.user.id, teamId } }
    })

    if (!membership || membership.role !== "MANAGER") {
        return next(new AppError("Only manager of this team can do that", 403, "NOT_TEAM_MANAGER"))
    }
