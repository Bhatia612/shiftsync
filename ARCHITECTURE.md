...
The backend is Node and Express talking to a PostgreSQL database through Prisma. The frontend is React with Vite, and it's installable as a PWA. They talk over a plain JSON API, and the user's login lives in an httpOnly cookie so the browser sends it automatically and no token ever touches JavaScript.
...
Because a manager is also a regular worker with their own shifts, the frontend has a mode switch: a manager can flip between a manager view (the team schedule, the approval queue, roster management) and an employee view (their own shifts and their own swap requests). The mode is a frontend concern - the person is always a manager in the data - and it's persisted so it survives a refresh. A manager in employee mode is treated, in the UI, like any other employee.
...
When a swap resolves, the request records who resolved it, so the history can show the actual person - "approved by Alex" rather than a faceless "approved."

## Keeping the schedule sane

Two rules protect the schedule from ending up in a state that doesn't make sense.

**No one gets double-booked.** Before a shift is assigned to someone, the system checks whether they already have a shift that overlaps that time. This is a check in code rather than a database rule, because "these two time ranges overlap" isn't something a simple database constraint can express. This check runs twice: when a swap is first requested (so you can't even propose someone who's already scheduled) and again right before approval (in case the schedule changed while the request was waiting).

**Approval happens all at once.** When a manager approves a swap, the shift gets reassigned and the request gets marked resolved in a single database transaction. Either both happen or neither does. There's no window where the shift moved but the request still looks open, or the reverse. Validation - checking the transition is legal and the target is still free - happens before the transaction; only the actual mutations run inside it.

There's a known gap here worth naming: because a shift isn't locked when a swap is requested, two separate swaps can target the same shift, and approving the second one after the first can overwrite the result. The planned fix distinguishes targeted swaps (a single chosen teammate, where no race is possible) from broadcast swaps (multiple candidates, where the race is resolved at approval time by auto-declining the ones that lose).

## Live notifications

When something happens to a swap - requested, accepted, approved, denied, cancelled - the people involved find out right away. That's done with Server-Sent Events, a one-way stream from the server to the browser.
...
Every notification is saved to the database first, then pushed live to any open connection for that user. So someone who's offline doesn't miss anything - the notification is waiting for them when they return, and the live push is a bonus for whoever's connected right now. On the frontend, a single stream is opened while the user is logged in; incoming events update the notification list and also refresh any swap data on screen, so an open page reflects a change without a manual reload.

Managers are kept in the loop from the start: they're notified when a swap is first initiated (for awareness) and again when it's accepted and reaches their queue (where they can act).

## The frontend

The React app is organized by feature - auth, teams, schedule, requests, notifications - rather than by file type. Everything that makes the schedule work sits together in the schedule folder, instead of being split across a components pile, a hooks pile, and a services pile. When you're working on one part of the app, the pieces you need are in one place.
...
The frontend is hosted on Vercel, the backend on Render, and the database on Neon.

Because the frontend and backend sit on different domains, the auth cookie would count as a third-party cookie - and privacy-focused browsers block those by default, which quietly breaks login on mobile. The fix is to route API calls through the frontend's own domain: the frontend proxies `/api` to the backend, so from the browser's point of view every request is same-origin and the cookie stays first-party. In production the cookie is set with `SameSite=None; Secure`, and CORS is limited to the frontend origin.
